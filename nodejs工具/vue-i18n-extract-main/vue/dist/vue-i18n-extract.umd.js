(function (global, factory) {
  ...
  function readVueFiles(src) {
    ...
    const normalizedSrc = [
      'D:/Users/Desktop/vue-i18n-extract-main/vue-i18n-extract-main/tests/fixtures/vue-files/**/*.js',
      'D:/Users/Desktop/vue-i18n-extract-main/vue-i18n-extract-main/tests/fixtures/vue-files/**/*.vue'
    ]
    ...
    let targetFiles = [];
    normalizedSrc.forEach(item => {
      targetFiles = [...targetFiles, ...glob__default["default"].sync(item)]
    })
  ...
  function* getMatches(file, regExp, captureGroup = 1) {
    while (true) {
      ...
      const grounpName = path ? path.split('.')[0] : ''
  ...
  function readLanguageFiles(src) {
    ...
    return targetFiles.map(f => {
      ...
      if (isJSON) {
        ...
        let file = fs__default["default"].readFileSync(langPath, 'utf8')
        file = file.slice(-1) === ';' ? file.slice(0, -1) : file;
        const _file = file.replace(/export default\s+/, 'module.exports = ');
        langObj = eval(_file);
      }
  ...
  function extractI18NReport(vueItems, languageFiles, detect) {
    ...
    Object.keys(languageFiles).forEach(language => {
      ...
      if (detect.includes(exports.DetectionType.Unused)) {
        const unusedKeysInLanguage = languageItems.filter(languageItem => 
          !vueItems.some(vueItem => 
            languageItem.path === vueItem.path || languageItem.path.startsWith(vueItem.path + '.'))
          ).map(languageItem => 
              _extends({}, languageItem, { language }));
  ...
  async function writeReportToFile(report, writePath) {
    const reportString = JSON.stringify(report.unusedKeys);
  ...
  async function createI18NReport(options) {
    ...
    // const testFiles = ['./tests/fixtures/vue-files/**/*.js', './tests/fixtures/vue-files/**/*.vue']
    // const vueFiles = []
    // testFiles.forEach(file => {
    //   vueFiles.push(readVueFiles(path__default["default"].resolve(process.cwd(), file)));
    // })
