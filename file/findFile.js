                    ...
                    if (eror) {
                        ...
                        if (isFile && filedir.split('.')[1] === 'vue') {
                            ...
                            fs.readFile(filedir, 'utf8', function(err,dataStr) {
                              // 提取table中的button
                              const tableOpReg = /operation: \[([\s\S]*?)\]/g
                              const tableOp = dataStr.match(tableOpReg)?.[0]
                              const regTableName = /\$t\('(.*?)\.(.*?)'\)/g
                              tableOp && tableOp.replace(regTableName, (match, $1, $2) => {
                                const str = 'table' + $2
                              ...
                              const buttons = dataStr.match(reg)
                              // 提取button的名字
                              const regName = /\$t\('(.*?)\.(.*?)'\)/g
